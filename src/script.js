class NeuralNetwork {
    constructor(inputSize, hiddenSize, outputSize) {
        ...
        this.learningRate = parseFloat(document.querySelector('#learningRate').value); // Adjusted learning rate
    ...
    train(inputs, target) {
        const hiddenLayer = this.feedForward(inputs);
        const errorsOutput = new Array(this.outputSize);
        const errorsHidden = new Array(this.hiddenSize);

        for (let i = 0; i < this.outputSize; i++) {
            errorsOutput[i] = target[i] - hiddenLayer[i];
            for (let j = 0; j < this.hiddenSize; j++) {
                this.weightsHiddenToOutput[i][j] += this.learningRate *
                    errorsOutput[i] * hiddenLayer[i] * (1 - hiddenLayer[i]) * this.hiddenLayer[j];
            }
        ...
        for (let i = 0; i < this.hiddenSize; i++) {
            ...
            for (let j = 0; j < this.inputSize; j++) {
                this.weightsInputToHidden[i][j] += this.learningRate *
                    errorsHidden[i] * this.hiddenLayer[i] * (1 - this.hiddenLayer[i]) * inputs[j];
            }
        }
    }
}
...
const trainingData = [];
const numDataPoints = parseInt(document.querySelector('#trainingDataSize').value); // Adjust the number of data points as needed
...
function drawInputToHiddenWeights(inputX, hiddenY) {
    for (let i = 0; i < neuralNetwork.inputSize; i++) {
        for (let j = 0; j < neuralNetwork.hiddenSize; j++) {
            ...
            const toX = 300;
            const toY = 100 + (j * 100);
            drawWeight(fromX, fromY, toX, toY, weight, getWeightColor(weight));
        }
    }
}
...
function getWeightColor(weight) {
    const normalizedWeight = Math.tanh(weight); // Normalize weight using tanh
    const hue = (1 - normalizedWeight) * 120; // Convert to hue (green to red)
...
function drawBias(x, y, bias) {
    ctx.font = '12px Arial';
    ctx.fillStyle = 'black';
    ctx.fillText(`Bias: ${bias.toFixed(2)}`, x, y + 30); // Adjust the position
}
...
function drawWeight(fromX, fromY, toX, toY, weight, color) {
    ...
    ctx.font = '14px Arial';
    ctx.fillStyle = color;
    ctx.fillText(weight.toFixed(2), midX, midY + 10); // Display weight value
}
